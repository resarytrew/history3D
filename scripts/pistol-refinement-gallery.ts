...
const before=JSON.parse(await readFile(resolve(out,'relief-before/metrics.json'),'utf8'))
...
const comparisons=[
  {name:'lock',title:'Крупный план замка',source:'detail-3.jpg',note:'Углублённые прорези головок, ребристый участок винта, гранёная шейка, поднятый выступ верхней губки, ступень огнива, ободок большой головки, край полки и низкий рельеф курка. Нейтральное железо без искусственного старения.'},
...
<p><a href="../../PISTOL_REFINEMENT_REPORT.md">Отчёт и ограничения</a> · <a href="relief-review.json">Запись текущего визуального прохода</a> · <a href="acceptance-audit.json">Рендеры и контрольные суммы</a> · <a href="/?exhibit=russian-pistol-1798-1804">Экспонат</a></p>
<table><tr><th>Метрика</th><th>До объёмных деталей</th><th>После</th></tr><tr><td>Треугольники</td><td>${before.triangles}</td><td>${after.triangles}</td></tr><tr><td>Meshes</td><td>${before.meshes}</td><td>${after.meshes}</td></tr><tr><td>GLB, байты</td><td>${before.bytes}</td><td>${after.bytes}</td></tr></table>
${comparisons.map(({name,title,source,note})=>`<section><h2>${title}</h2><p>${note}</p><div class="comparison">${picture(sources+source,'Музейный оригинал')}${picture(`relief-before/photo-${name}.png`,'До объёмных деталей')}${picture(`photo-${name}.png`,'После')}</div><div class="overlay"><img src="${sources+source}" alt="Источник"><img id="overlay-${name}" src="photo-${name}.png" alt="Рендер поверх источника"></div><label class="slider">Фото<input data-overlay="overlay-${name}" type="range" min="0" max="100" value="50" aria-label="Прозрачность рендера: ${title}">Модель</label></section>`).join('')}
