...
const before=JSON.parse(await readFile(resolve(out,'lock-before/metrics.json'),'utf8'))
...
const comparisons=[
  {name:'lock',title:'Крупный план замка',source:'detail-3.jpg',note:'Выпуклые поверхности курка и огнива, округлая головка с узкой шейкой, ободок большого винта и задние рёбра. Отдельная фактура железа и поверхностное начертание ТУЛА / 1803 по снимку.'},
...
<p>Главные ориентиры — три фотографии, предоставленные пользователем: оба боковых вида и крупный план замка. Рис. 128 Маковской относится к базовому типу и не заменяет очертания данного предмета. Последний проход сосредоточен на наружном рельефе замка. Общие профили, камеры и интерфейс сохранены. <a href="profile-comparison.json">Проверка выбранных контурных точек</a>.</p>
...
<p><a href="../../PISTOL_REFINEMENT_REPORT.md">Отчёт и ограничения</a> · <a href="refinement-audit.json">Аудит геометрии</a> · <a href="acceptance-audit.json">Рендеры и контрольные суммы</a> · <a href="/?exhibit=russian-pistol-1798-1804">Экспонат</a></p>
<table><tr><th>Метрика</th><th>До доводки замка</th><th>После</th></tr><tr><td>Треугольники</td><td>${before.triangles}</td><td>${after.triangles}</td></tr><tr><td>Meshes</td><td>${before.meshes}</td><td>${after.meshes}</td></tr><tr><td>GLB, байты</td><td>${before.bytes}</td><td>${after.bytes}</td></tr></table>
${comparisons.map(({name,title,source,note})=>`<section><h2>${title}</h2><p>${note}</p><div class="comparison">${picture(sources+source,'Музейный оригинал')}${picture(`lock-before/photo-${name}.png`,'До доводки замка')}${picture(`photo-${name}.png`,'После')}</div><div class="overlay"><img src="${sources+source}" alt="Источник"><img id="overlay-${name}" src="photo-${name}.png" alt="Рендер поверх источника"></div><label class="slider">Фото<input data-overlay="overlay-${name}" type="range" min="0" max="100" value="50" aria-label="Прозрачность рендера: ${title}">Модель</label></section>`).join('')}
<section><h2>26 контрольных ракурсов</h2><div class="gallery">${pistolControlViews.map(v=>picture(`${v.name}.png`,v.name==='22-jaws-flint'?'22 · Пустые губки и головка винта':v.name)).join('')}</div></section>
<p>Неизвестные толщины, сечения и невидимые поверхности остаются авторской реконструкцией. Древесные PBR-карты сохранены из предыдущей версии: Фактура железа и начертание надписи приближённые; индивидуальные повреждения оригинала не скопированы. Стопроцентное сходство не заявляется.</p>
