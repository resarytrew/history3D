...
test('musket loads, opens five details, resets and switches exhibits', async ({ page }, testInfo) => {
  ...
  if (await compare.isVisible()) {
    ...
    await expect(canvas).toHaveAttribute('data-contact-shadow','hidden')
  ...
  await expect(page.locator('.viewer-stage')).toHaveAttribute('data-state', 'ready', { timeout: 30_000 })
  await expect(canvas).not.toHaveAttribute('data-contact-shadow',/./)
...
test('musket markers follow rotation, hide behind the object and return on reset', async ({ page }, testInfo) => {
  ...
  // Inspect the underside: the display shadow must not cover the artifact.
  await page.mouse.move(startX,bounds.y+bounds.height*0.45); await page.mouse.down()
  await page.mouse.move(startX,bounds.y+bounds.height*0.12,{steps:16}); await page.mouse.up()
  await expect(canvas).toHaveAttribute('data-contact-shadow','hidden')
  await page.screenshot({path:`docs/verification/russian-musket-1808/underside-${testInfo.project.name}.png`})
  await page.getByRole('button', { name: 'Сбросить ракурс' }).click()
  await expect(canvas).toHaveAttribute('data-contact-shadow','visible')
})
