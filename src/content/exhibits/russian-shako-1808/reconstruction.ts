...
export const shakoInferred: readonly EvidenceItem[] = [
  ...
  { id: 'model-thickness', kind: 'RECONSTRUCTION', statement: 'Толщина кожи 2,5 мм, козырька 3,5 мм, диаметр пряди шнура 3,2 мм; микрокривизна и допуски — технические приближения.', sourceIds: [], confidence: 'medium' },
  { id: 'identity-proxy', kind: 'RECONSTRUCTION', statement: 'Гренада около 73 × 29 мм: пропорции контура, продольные рёбра пламени и два поперечных пояска восстановлены по предоставленной фотографии накладки. Толщина листа 0,45 мм и высота рельефа около 0,95 мм не измерены. Репеёк 58 × 35 мм и траектории плетения остаются приближениями.', sourceIds: ['archaeolog-one-flame', 'locked-shako-spec'], confidence: 'medium' },
