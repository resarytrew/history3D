...
export const shakoInferred: readonly EvidenceItem[] = [
  ...
  { id: 'identity-proxy', kind: 'RECONSTRUCTION', statement: 'Знак высотой около 73 мм и репеёк 78 × 43 мм — пропорциональные ориентиры. Контур гренады и траектории плетения восстановлены по словесной спецификации: приложенные изображения не дают точного фронтального образца этого знака.', sourceIds: ['locked-shako-spec'], confidence: 'low' },
...
export const shakoUnknown: readonly EvidenceItem[] = [
  { id: 'badge-measurement', kind: 'UNKNOWN', statement: 'TODO_RESEARCH: точные размеры и контур конкретного сохранившегося знака не установлены. Требуется историческая проверка человеком без автоматической подмены заданных данных.', sourceIds: [], confidence: 'unknown' },
...
export const shakoReconstruction: Exhibit['reconstruction'] = {
  ...
  versions: [{ version: '0.1.0', date: '2026-09-06', summary: 'Процедурная геометрия, размерные тесты и фиксированные ракурсы. PENDING HUMAN REVIEW.', status: 'historical-review' }],
}
