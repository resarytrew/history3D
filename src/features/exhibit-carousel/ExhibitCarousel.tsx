...
export function ExhibitCarousel({ collection, activeId, thumbnails, onSelect }: ExhibitCarouselProps) {
  const ui = useUi()
  return (
    <nav className={`exhibit-carousel ${collection.entries.length === 1 ? 'is-single' : collection.entries.length === 2 ? 'is-pair' : collection.entries.length === 3 ? 'is-pair is-trio' : ''}`} aria-label={ui.exhibits}>
